    ...
    public class AnarchyCommands
    {
        ...
        public void SendToAnarchy()
        {
            ...
            try
            {
                ...
                // Use AutoCAD SAVEIMG command to export the current viewport
                // SAVEIMG prompts: filename, format, background, size — we automate via script
                doc.SendStringToExecute(
                    "_SAVEIMG\n" + tempPath + "\nBMP\n\n",
                    true, false, false);

                // Wait for SAVEIMG to finish (up to 5 seconds)
                int waited = 0;
                ...
                // Convert BMP to PNG to reduce size
